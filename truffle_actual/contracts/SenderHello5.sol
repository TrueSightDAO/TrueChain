...
contract SenderHello5 {

    mapping(address => HelloMessage5) _messages;

    function createMessage() public {
        require ( _messages[msg.sender] == HelloMessage5(address(0)), "Message is already created" );
        _messages[msg.sender] = new HelloMessage5(msg.sender);
    }

    function setMessage(string memory new_message) public {
        require (_messages[msg.sender] != HelloMessage5(address(0)), "Message not created yet" );
        HelloMessage5(_messages[msg.sender]).setMessage(msg.sender, new_message);
    }

    function getMessage(address account) public view returns (string memory) {
        require (_messages[account] != HelloMessage5(address(0)), "Message not created yet" );
    ...
    function validateMessage(address account) public {
        require (_messages[account] != HelloMessage5(address(0)), "Message not created yet" );
        HelloMessage5(_messages[account]).validate();
    }

    function invalidateMessage(address account) public {
        require (_messages[account] != HelloMessage5(address(0)), "Message not created yet" );
        HelloMessage5(_messages[account]).invalidate();
    }

    function isValidated(address account) public view returns (string memory)  {
        require (_messages[account] != HelloMessage5(account), "Message not created yet" );
        if (HelloMessage5(_messages[account])._validated()) {
          return "Validated";
        } else {
