...
contract SenderHello2 {

    mapping(address => HelloMessage2) _messages;

    function createMessage() public {
        require ( _messages[msg.sender] == HelloMessage2(address(0)), "Message is already created" );
        _messages[msg.sender] = new HelloMessage2(msg.sender);
    }

    function setMessage(string memory new_message) public {
        require (_messages[msg.sender] != HelloMessage2(address(0)), "Message not created yet" );
        HelloMessage2(_messages[msg.sender]).setMessage(msg.sender, new_message);
    }

    function getMessage(address account) public view returns (string memory) {
        require (_messages[account] != HelloMessage2(address(0)), "Message not created yet" );
